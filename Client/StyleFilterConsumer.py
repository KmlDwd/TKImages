...
from StyleFilter.StyleFilter import process_request
from Utils.Utils import setup_health_consumer

logger = logging.getLogger("StyleFilterConsumer")
...
SERVICE_NAME = "style_service"

if __name__ == '__main__':
    logger.info("Starting StyleFilterConsumer")
    consumer = RabbitMQSyncConsumer.from_config('style')
    producer = RabbitMQProducer.from_config()
    health_consumer = RabbitMQAsyncConsumer.from_config('health')
    logger.info("StyleFilterConsumer started successfully")
